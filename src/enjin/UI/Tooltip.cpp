...
C_Tooltip::C_Tooltip(Object *owner, int8_t precision) : Component(owner),
                                                         precision(precision),
                                                         value(220.0f),
                                                         width(31),
                                                         origin(0, 0),
...
void C_Tooltip::Draw(GFXcanvas8 &canvas)
{
    if (is_active)
    {
        ...
        // draw canvas
        canvas.drawGrayscaleBitmap(origin.x, origin.y, this->canvas.getBuffer(), (uint8_t)7, this->canvas.width(), this->canvas.height());
    }
}
